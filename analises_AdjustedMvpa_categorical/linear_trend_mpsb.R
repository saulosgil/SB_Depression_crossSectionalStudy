...
df2 <-
  ...
  mutate(
    ...
    # Quintis de SB (1 a 5)
    sb_q_num = ntile(total_sb_mp_hday, 4),
...
m_adj_trend <- lm(
  dep_log ~ sb_q_num + idade_cat + genero + raca + renda +
    has + dm2 + obesidade + ativo_status,
  data = df2
)
...
m_adj_q <- lm(
  dep_log ~ sb_q + idade_cat + genero + raca + renda + has + dm2 + obesidade + ativo_status,
  data = df2
)
...
gm_table <- function(model, label) {
  em <- emmeans(model, specs = "sb_q", rg.limit = 12288) |> as.data.frame()
